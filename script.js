...
function getDateRange(filter) {
  const now = new Date('2025-07-26T23:10:00+05:30'); // Updated to current date and time: July 26, 2025, 11:10 PM IST
  let start, end;
...
function parseBillDate(timestamp) {
  try {
    ...
    if (!timestamp || typeof timestamp !== 'string') {
      console.warn('Invalid or missing timestamp, using current date');
      return new Date();
    }
    const parts = timestamp.match(/(\d{1,2})\/(\d{1,2})\/(\d{4}),\s*(.*)/);
    if (!parts) {
      console.warn('Invalid timestamp format, falling back to Date constructor:', timestamp);
      return new Date(timestamp);
    }
    const dateString = `${parts[2]}/${parts[1]}/${parts[3]} ${parts[4]}`;
    const parsedDate = new Date(dateString);
    if (isNaN(parsedDate.getTime())) {
      console.warn('Invalid date parsed, falling back to current date:', timestamp);
      return new Date();
    }
...
function showBillsView() {
  ...
  let storedBills = [];
  try {
    storedBills = JSON.parse(localStorage.getItem('bills')) || [];
  } catch (error) {
  ...
  if (storedBills.length === 0) {
    ...
    storedBills.forEach(bill => {
      if (!bill || !bill.transactionId || !bill.items || !Array.isArray(bill.items)) {
      ...
      bill.items.forEach(item => {
        if (!item || !item.name || typeof item.quantity !== 'number' || typeof item.price !== 'number') {
          console.warn('Skipping invalid item in bill:', item);
          return;
        }
        itemDetailsRows += `
          <tr>
      ...
      tableRows += `
        <tr class="bill-summary-row" data-transaction-id="${bill.transactionId}">
          ...
          <td>$${bill.payable ? bill.payable.toFixed(2) : '0.00'}</td>
        </tr>
...
function showSalesView() {
  console.log('showSalesView called');
  try {
    selectedCategory = "--Select--"; // Reset filter to show all bills
    leftPanel.style.display = 'none';
    rightPanel.style.display = 'none';
    billsViewPanel.style.display = 'none';
    ...
    try {
      ...
      console.log('Raw bills data from localStorage:', billsData);
      if (billsData) {
        storedBills = JSON.parse(billsData);
        if (!Array.isArray(storedBills)) {
          console.warn('Bills data is not an array, resetting to empty array');
          storedBills = [];
      ...
      console.error('Error parsing bills from localStorage:', error);
      storedBills = [];
      localStorage.setItem('bills', JSON.stringify(storedBills)); // Reset corrupted data
    }
      ...
      <div class="sales-filter-container">
        <div class="custom-dropdown" id="sales-category-filter">
          ...
          <div class="dropdown-menu">
            <div class="dropdown-item selected" data-value="--Select--">--Select--</div>
            <div class="dropdown-item" data-value="Today">Today</div>
            <div class="dropdown-item" data-value="Yesterday">Yesterday</div>
            <div class="dropdown-item" data-value="This Week">This Week</div>
            <div class="dropdown-item" data-value="Last Week">Last Week</div>
            <div class="dropdown-item" data-value="This Month">This Month</div>
            <div class="dropdown-item" data-value="Last Month">Last Month</div>
            <div class="dropdown-item" data-value="Custom">Custom</div>
          </div>
        </div>
        <div class="date-filter" id="sales-date-filter" style="display: none;">
          <input type="date" id="sales-from-date" aria-label="From date" value="">
          <input type="date" id="sales-to-date" aria-label="To date" value="">
        </div>
      </div>
    ...
    if (storedBills.length === 0) {
      ...
      storedBills.forEach(bill => {
        try {
          if (!bill || !bill.items || !Array.isArray(bill.items) || !bill.timestamp) {
            console.warn('Skipping invalid bill:', bill);
            return;
          }
      ...
      if (salesEntries.length === 0) {
        ...
        content = content.replace('<tbody id="sales-table-body"></tbody>', `<tbody id="sales-table-body">${tableRows}</tbody>`);
      }
    }
    ...
    console.error('Critical error in showSalesView:', error);
    salesViewPanel.innerHTML = `
      <div class="sales-header">
        <h2>Sales Summary</h2>
        <button class="retry-button" id="back-to-pos-from-sales">Back to POS</button>
      </div>
      <p class="error-message">Error loading sales data. Please try again or clear local storage.</p>
    `;
    document.getElementById('back-to-pos-from-sales').addEventListener('click', showMainView);
  }
}
...
goodsReturnButton.addEventListener('click', () => {
  console.log('Goods Return button clicked');
  showSalesView();
});
